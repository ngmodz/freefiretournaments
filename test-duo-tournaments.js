...
import { createDuoTeamForTournament, validateDuoTeam } from './src/lib/teamService.ts';
import { joinTournamentAsTeam } from './src/lib/tournamentService.ts';
import { distributePrize } from './src/lib/prizeDistributionService.ts';
...
async function createAndJoinDuoTeams() {
  ...
  for (let i = 0; i < TEST_CONFIG.duoTeams.length; i++) {
    ...
    try {
      ...
      await joinTournamentAsTeam(createdTournamentId, teamData, leaderId);
  ...
  try {
    ...
    for (let i = 0; i < winners.length; i++) {
      ...
      if (prizeAmount > 0) {
        ...
        await distributePrize(createdTournamentId, winner.leaderId, position, prizeAmount);
